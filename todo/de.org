...
** compiz viewports
   2016-08-19 22:45:01 Fri NZST
   有机会最好可以分开处理view-ports和普通工作区.
   现在是把这两种混淆在一起处理.
   一些特殊逻辑是,view-ports判断窗口是否在同一个工作区的办法是依据坐标范围的
   因此,一个半露的view-ports窗口,很可能是来自旁边的工作区的.
*** 移动半露窗口
    现在有一个问题是,一个看不见的窗口,是否允许对它进行move,
    如果这是一个半露的窗口的话,其实是很需要的,按照我们的经验来说
    看到一个半露的窗口就会直觉的想要移动它.
    这个问题其实是超出tile以外的,只是单纯的窗口操作.
** 判断float和tile的基准
   现在我们有两种模式,float和tile,区分的基准是看是否存在重叠的窗口.
   如果说不建立全局隐变量的话,还会有更好的判断模式吗?
*** 学习?
    在早期使用的时候,对不确定的布局进行询问,来获取用户操作习惯？
    很有趣的想法．
    但是,很显然用户是会犯错,或者说是概率性的在特定的布局情况下会偶然想要用另一种模式.
    如果说收集大量用户数据来获取统一的结果的话,
    这个结果比起大量统计,或许是可以简单推理计算的.
    这个我们之后再看,是否最合理的模式判断依据是不可简单推理计算的.
*** 情况分析
    现在,处理单一窗口,是floating模式,
    这个时候开一个新窗口,大脑会下意识的认为是floating模式,
    但是因为没有重叠,程序判断为tile模式.
    大脑则是对严格布局的tile,才会判断为tile模式.

    - 特殊化的情况是,对于一个floating布局,会希望转为tile模式.
      因此才调用tile脚本做regularize
      还有一种情况是,在tile内部,微调了一个窗口,但是仍然把当前模式视为tile
    - 与之矛盾的情形是,其中最极端的是,双浮动窗口的情形,
      散乱放两个floating窗口是很常见的,而且很可能它们没有重叠.
    - 同样是两个窗口,
      tile模式下,微调了其中一个,这个时候对大脑来说应该还是tiling mode
      floating mode下,新建两个窗口,一般不会重叠,但是对大脑来说是floating mode
      一般来说,后者的散乱度会很大程度的超出前者,
      但是散乱度的确是个很模糊的概念.
      具体来说得话,第一种情形中,两个窗口的坐标匹配度会很高,很有可能其中一个数值是完全相等的,或者非常接近.并且很多情形下,两个窗口应该会占据整个桌面
      第二种情形,很有可能仅仅占据桌面的很小一部分,但是的确,新建的两个窗口的坐标还是有可能匹配的,这个桌面的新建窗口的机制有关.所以两个窗口的整体位置可能是大脑区分两种mode的关键.
    - 结论
      或许这两种情形对于大脑来说在不依赖隐变量的情形下,在很多情形下并不是互相矛盾的.
      不过大脑对此的区分规则似乎的确会很模糊很复杂.
      所以或许machine learning的确是在这个问题上适用的.
      也就是找出大脑如何判断划分一个layout为tiling mode还是floating mode
      这个规则应该无法用简单明晰的数学来规划.
    - 对外说明
      如果你实现了这样一个ML功能要如何对外说明,
      对不同的layout截图,然后让人直觉判断会进行tiling操作还是floating操作吗?
      这很难描述,操作过程中,人的想法是很直观的,但是我不觉得人看到layout的截图的
      时候会产生区分mode的想法.
*** 开放api
    现在的基准完全是overlapping
    如上所述,可能应该更多的从整体桌面布局去考虑
    但是这个规则很复杂很模糊,
    但是即使如此,少量的数学公式应该还是足以描述绝大多数情形的.
    所以,用一些针对layout的数学公式和overlapping联用的话,组成一个api
    这样用户就可以自己设定区分floating和tiling的标准了
    其实我现在可以想到一个简单规则,
    就是layout中是否有窗口的四角和屏幕的四角对应?四个角中有两个可以对应上的话,
    是tiling的可能性就会很高.
    如此的一个规则,应该足以让程序在绝大多数时候做出和大脑相匹配的判断.
    
    虽然这个规则很简单,并且精度很高,但是从含义上来说应该是很模糊,无意义的
    我仍然觉得这个问题属于ML领域,只是恰好存在着一个可以简单而精确描述下来的规则.
* panel
** 位置
...
**** 模仿
**** 白名单
     把黑名单改成白名单,看似还是那么几个程序,
     但是似乎流畅不少.
     原因可能是某些无法看到的程序占掉了些性能
     但是尤其conky和yabar这样的跨桌面程序,在白名单中也会导致转换桌面的时候出现耳机噪音(也就是性能消耗较大吧)
*** blur compton对比
...
*** wallpaper插件位移
    2016-08-17 22:26:16 Wed NZST
    记得从0.9到0.8,wallpaper插件在切换桌面的时候替换wallpaper的方式都不是位移.
    不过最新的在笔记本上编译的compiz0.8似乎换成位移了.
    但是台式机上重新编译后,还不是.或许问题和nvidia有关?

    - 最后测试结果是当前aur的compiz-core是位移效果,但是compiz-core-git却不是.
      这个版本是0.8.12.3
    - 确认影响的因素超出了程序编译和配置之外.
      在一个全新编译,零配置的环境中,原本使没有位移的,但是关闭wall插件然后打开之后,
      就会有位移效果了.
      也就是说和wall的启动顺序有关.
    - 然后确认,在运行中的环境中,清空compiz配置后,也可以实现位移.
    - 最后确认,除了plugins列表以外,其他配置没有造成影响.
      列表的问题是如此的
      - imgjpeg;是问题最大的,不过幸好用不上.
      - ;expo;switcher;animation;dbus;regex;commands;
	这部分插件存在的话,需要wall重启
      - core;ccp;move;resize;place;decoration;png;wallpaper;blur;staticswitcher;
	这部分插件基本事没有问题的.
      - wall自身,
	似乎无法在一开始就开启,而需要之后手动从ccsm中开启.
      因此方案是,关掉imgjpeg,然后想办法用程序自动重启wall插件. 
      基本可以确定,重置起效后一次session内不会再犯这个问题.
*** compiz-core-git不稳定
    这个版本不稳定,比如和emerald联用就会出sig错误.
* theme
** gtk
...
** source
   ...
   可以在youtube 上找找歌曲背景
** 布局
   2016-08-20 02:11:14 Sat NZST
   布局和orientation
   比较理想的布局是主景在右侧,左侧留空
   因为文字是从左朝右排布的,所以文字一般都会在右侧留空.
   这样就正好互补了.
*** rotation
    做了rotation之后,应该把右侧作为top,因为如果原景是横向的主景会在top部分,也就是靠右.
*** 单调
    不过维持所有的主景在右侧其实感觉太单调了,
    还是维持原图随意的布局比较好.
* Grids
  2016-08-22 20:12:59 Mon NZST
  如果reimpl viewports 的话需要做什么?
** 快捷设定当前桌面壁纸
   现在的wallpaper插件可以很方便设定随机壁纸
   但是却没有方便的设定当前桌面的壁纸.
   有的时候会在特定工作区,需要特定的亮色或者暗色壁纸
** opengl
*** 飘动的水滴
    2016-08-22 17:04:27 Mon NZST
    飘动的水滴效果.水滴中可以嵌入文字,图标
    这单纯就是美化,很难说是有什么特殊功用.功用可能接近conky或者docky
    不过DE的发展中很大程度就是为了美化
**** 可读性
     如果是用作文字修饰的话,可读性很重要.
**** 实现
     因为涉及到融合不同窗口,因此几乎肯定只能用compositor来做
**** 设计
     要创造一个统一的设计原则,才能应用到不同元素.
